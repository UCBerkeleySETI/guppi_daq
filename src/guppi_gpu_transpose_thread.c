/* guppi_gpu_transpose_thread.c
 *
 * Transpose data on the GPU.
 */
...
void guppi_gpu_transpose_thread(void *_args) {
    ...
    if (rv<0) { 
        guppi_error("guppi_gpu_transpose_thread", "Error setting cpu affinity.");
    ...
    if (rv!=0) {
        guppi_error("guppi_gpu_transpose_thread", "Error setting priority level.");
        perror("set_priority");
    }
    ...
    if (rv!=GUPPI_OK) {
        guppi_error("guppi_gpu_transpose_thread", 
    ...
    if (db_in==NULL) {
        ...
        sprintf(msg, "Error attaching to databuf(%d) shared memory.",
                args->input_buffer);
        guppi_error("guppi_gpu_transpose_thread", msg);
        pthread_exit(NULL);
    }
    ...
    while (run) {
        ...
        if (first) {
            ...
            /* Downsample params */
            ds.dsfac = 1; //pf.hdr.ds_time_fact;
        ...
        // 3. After processing all the channels, transfer the data back onto
        //    the host directly into the output datablock.
...
#endif
        // Now copy the data directly back into 
        // output data block
        transfer_to_host(&ds, ds.nchan*ds.npol*npts_block, curdata_out);
